#Author: your.email@your.domain.com
#Keywords Summary :
#Feature: List of scenarios.
#Scenario: Business rule through list of steps with arguments.
#Given: Some precondition step
#When: Some key actions
#Then: To observe outcomes or validation
#And,But: To enumerate more Given,When,Then steps
#Scenario Outline: List of steps for data-driven as an Examples and <placeholder>
#Examples: Container for s table
#Background: List of steps run before each of the scenarios
#""" (Doc Strings)
#| (Data Tables)
#@ (Tags/Labels):To group Scenarios
#<> (placeholder)
#""
## (Comments)
#Sample Feature Definition Template

Feature: Sign-on Page test
Registered user should be able to login from sigon page as well after providing valid credentials

@EndToEnd, @Regression
Scenario: Positive login functionality test from Sign-on page
	Given registered user is in sign-on page
	When user provides valid username and valid password
	And user clicks on submit button
	Then user should be able to login and see login success message

@Smoke, @Regression
Scenario: Sigon Page element testing
	Given registered user is in sign-on page
	Then user should be able to see username textfield, password textfield and submit button
	And username textfield, password textfield and submit button should be functional all times